...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Themes-5%20Presets-FF7675" alt="Themes" />
...
项目打破了传统 Electron 客户端单调的视觉体验，在完全不影响原有核心编码与 AI 交互功能的前提下，注入了五套极具艺术张力与实用性的定制主题。同时采用**非侵入式加载器架构**，结合自研的 CDP（Chrome DevTools Protocol）通讯通道，实现了**无需重启客户端、0.1 秒全窗口无感秒级切换**。
...
1. **⚡ 0 重启秒级热切换 (Zero-Restart CDP Hot-Switching)**
   - 内置 `switch.bat` / `switch-theme.ps1` (Windows) 及 `switch.sh` (macOS)，自动通过本地 DevTools 端口建立调试连接。
...
3. **🪟 自绘像素级无缝窗口按钮 (Windows) 与 macOS 原生交通灯融合**
   - 彻底绕过 Windows 11 `titleBarOverlay` 无法自定义底色并偶现浅白条闪烁的系统缺陷，采用自绘按钮完美融入顶栏。
   - macOS 下自动适配 Traffic Lights 原生位置与窗口留白。
4. **🛡️ 非侵入式安全架构**
   - 仅在 `app.asar` 中注入微型加载器钩子（数十行），所有主题 CSS 与字体资源完全存放在 asar 外部目录（`resources/<theme>/`）。
   - 注入器支持动态通用寻路，支持任意名称的新主题自动加载。
   - 用户可随时以纯文本方式修改 CSS，按下 `Ctrl + R` / `Cmd + R` 即可实时见效。
   - 安装前自动备份原始包（`app.asar.pixel-backup`），卸载脚本能够 100% 无损恢复原厂状态。
...
#### 1. 一键安装
1. **完全退出 Antigravity**（macOS 可在菜单栏退出或加 `-k`；Windows 加 `-KillRunning` 自动强退）；
2. 在本项目根目录下执行：
   ```bash
   # === macOS ===
   ...
   # 或指定安装主题：phantom | matcha | doodle | pixel | glass
   ./install.sh -t glass -k
   ```
   ```powershell
   # === Windows ===
...
```bash
# === macOS ===
./switch.sh         # 打开交互式数字选择菜单（按 1~5 快速选择）
./switch.sh phantom # 切换至 Persona 5 怪盗波普风
./switch.sh matcha  # 切换至 治愈系抹茶日记风
./switch.sh doodle  # 切换至 纯线稿漫画手绘风
./switch.sh pixel   # 切换至 8-Bit 复古像素风
./switch.sh glass   # 切换至 液体玻璃拟态风 [Demo版]
```
```cmd
:: === Windows (CMD / PowerShell 通用，或直接双击 switch.bat) ===
switch              :: 打开交互式数字选择菜单（按 1~5 快速选择）
switch phantom      :: 切换至 Persona 5 怪盗波普风
switch matcha       :: 切换至 治愈系抹茶日记风
switch doodle       :: 切换至 纯线稿漫画手绘风
switch pixel        :: 切换至 8-Bit 复古像素风
switch glass        :: 切换至 液体玻璃拟态风 [Demo版]
```
...
# macOS:
/Applications/Antigravity.app/Contents/Resources/<theme-name>/<theme-name>.css
```
- **实时调色**：用任意编辑器打开对应 `.css` 文件，修改顶部 `:root` 变量区（例如背景色、强调色、字体大小），保存后在 Antigravity 窗口按下 **`Ctrl + R`** (Windows) 或 **`Cmd + R`** (macOS) 即可在 1 秒内看到修改结果，无需重新安装！
...
1. **⚡ Zero-Restart Hot Switching (0.1s)**
   - Powered by `switch.bat` / `switch-theme.ps1` (Windows) and `switch.sh` (macOS) communicating via Chromium's local DevTools port.
...
3. **🪟 Seamless Titlebar Integration**
   - Eliminates Windows 11 `titleBarOverlay` tint discoloration and white background flash issues with custom self-drawn controls.
   - Native macOS traffic light window button integration.
4. **🛡️ Non-Invasive & Safe Architecture**
   ...
   - Dynamic theme fallback allows adding any custom theme folder without touching injector code.
   - Automatic `app.asar.pixel-backup` creation ensures complete uninstallation with `uninstall.bat` / `uninstall.sh`.
...
- **Dependency**: [Node.js](https://nodejs.org/) (Node 18+, `node` and `npx` must be available in PATH)

#### 1. Installation
1. **Completely exit Antigravity** (or use `-k` / `-KillRunning` to auto-terminate);
2. Run installation in terminal:
   ```bash
   # === macOS ===
   ./install.sh -k
...
```bash
# === macOS ===
./switch.sh         # Interactive number-based menu (1-5)
./switch.sh glass   # Direct switch
```
```cmd
:: === Windows ===
switch              :: Interactive number-based menu (1-5)
switch glass        :: Direct switch
```
...
├── glass-theme/             # Liquid glass theme CSS & SVG refraction lenses [Demo]
...
├── install.bat / .ps1 / .sh # Automated patcher & asset deployer (Windows / macOS)
├── switch.bat / .ps1 / .sh  # Instant zero-restart theme hot-switcher (Windows / macOS)
├── uninstall.bat / .ps1 / .sh # 100% clean restore & cleanup script (Windows / macOS)
